...
function play(seed,name){const p=strategies[name];let s=C.reduceGame(C.createRun({seed}),{type:'START_RUN',streetName:`Sim ${seed}`}),guard=0;const beats=[];while(s.run.status==='playing'&&guard++<500){s=settle(s,p,beats);if(s.run.status!=='playing')break;
    if(s.run.phase==='week_zero'){
      if(!s.nightOwl.boardViewedDays.includes(s.run.day)){s=C.reduceGame(s,{type:'VIEW_NIGHT_OWL_BOARD'});continue}
      if(!s.people.mara.met){s=C.reduceGame(s,{type:'VISIT_NIGHT_OWL'});continue}
      const regular=C.selectors.nightOwlRegularFor(s),regularState=s.nightOwl.regulars[regular.id];
      if(regularState.lastTalkDay!==s.run.day){s=C.reduceGame(s,{type:'TALK_NIGHT_OWL_REGULAR',regularId:regular.id});continue}
      if(!s.jobs.discovered.includes('wash_go')){s=C.reduceGame(s,{type:'WANDER_SPENARD'});continue}
      const job=C.selectors.jobAvailability(s,'wash_go');
      if(job.available){s=C.reduceGame(s,{type:'WORK_JOB',jobId:'wash_go',approach:p.mode==='legal'?'work_hard':'socialize'});continue}
