...
test("v3, v4, v5, and v6 saves all migrate to v7, and unsupported versions are refused", () => {
  // LEGACY_SAVE_KEYS promises every older schema still loads.
  assert.deepEqual(C.LEGACY_SAVE_KEYS, ["907ogr_v6", "907ogr_v5", "907ogr_v4", "907ogr_v3"]);
  for (const version of [3, 4, 5, 6]) {
    ...
    const state = C.hydrateRun(raw);
    assert.equal(state.version, 7, `v${version} save should migrate to v7`);
