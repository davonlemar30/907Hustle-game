...
function Header({ state, onMenu }) {
  ...
  return <header className="top">
    <h1 className="sr-only">907Hustle: One Good Run · v1.34</h1>
...
function phoneBills(state) {
  ...
  if (crew.length) {
    ...
    rows.push({ id: "wages", name: "Crew wages", amount: owed || crew.reduce((sum, person) => sum + person.wage, 0), where: "Pay in People → Crew", due: "Daily",
...
function MenuModal({ state, dispatch, onClose, onTitle }) {
  ...
  return <><Modal title="Run menu" onClose={onClose}>
    <ExpandableMoreSection
      ...
      expandedContent={<p className="popup-flavor">907Hustle v1.34 · Seed {state.run.seed} · Core v{state.version} · storage key {C.SAVE_KEY}</p>}
