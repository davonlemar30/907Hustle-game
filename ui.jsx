...
function Header({ state, onMenu }) {
  ...
  return <header className="top">
    <div className="brand-row"><div className="brand">907<b>Hustle</b><small>One Good Run · Alpha v0.8</small></div><button className="menu-btn" onClick={onMenu}>Menu</button></div>
...
function More({ state, dispatch, features }) {
  ...
  return <><PageHead title="More" sub="Character, finances, and help stay available; other systems open when the run gives them context" /><div className="scroll"><CategoryCard title="Character" status={(C.STREET_IDENTITIES[state.player.streetIdentity] || C.STREET_IDENTITIES.unproven).label} description="Street Identity, six attributes, derived ratings, and recent reputation." onClick={() => setPage("character")} /><CategoryCard title="Operations" status={features.operations.available ? "Available" : "Locked"} description={features.operations.available ? "Quick Score, Territory, Gear, and Safehouse." : features.operations.hint} disabled={!features.operations.available} onClick={() => setPage("operations")} /><CategoryCard title="Finances" status="Available" description={`${money(state.lender.balance)} remains on Dre's note. Review and make a payment.`} onClick={() => setPage("finances")} /><CategoryCard title="Recovery" status={features.recovery.available ? "Available" : "Locked"} description={features.recovery.available ? "Treat injuries or Lay Low to reduce Heat." : features.recovery.hint} disabled={!features.recovery.available} onClick={() => setPage("recovery")} /><CategoryCard title="Help" status="Available" description="Review time, trading, major actions, and the seven-day objective." onClick={() => setPage("help")} /></div></>;
}
...
function MenuModal({ state, dispatch, onClose, onTitle }) { function restart() { if (!window.confirm("Restart this run? The current autosave will be replaced after you confirm.")) return; dispatch({ type: "NEW_RUN", seed: Date.now() }); onClose(); } return <Modal title="Run menu"><p>Alpha v0.8 autosaves active play to <b>{C.SAVE_KEY}</b>.</p><button className="btn full primary" onClick={onTitle}>Return to Title</button><button className="btn full secondary choice" onClick={restart}>Restart Run<span>Creates a new seed and returns to Street Name entry.</span></button><button className="btn full secondary choice" onClick={onClose}>Close Menu</button><p className="muted">Autosave active · Seed {state.run.seed} · Core v{state.version} · {C.SAVE_KEY}</p></Modal>; }
